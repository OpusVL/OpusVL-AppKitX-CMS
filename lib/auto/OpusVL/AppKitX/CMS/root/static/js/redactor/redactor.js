...
(function($){
	...
	Redactor.prototype = {
		...
		pasteTextAtCaret: function (html)
		{
			var sel, range;
			if (document.getSelection)
			{
				...
				if (sel.getRangeAt && sel.rangeCount)
				{
					...
					range.deleteContents();
					var el = document.createElement("div");
					el.innerHTML = String(html).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');;
					var frag = document.createDocumentFragment(), node, lastNode;
					while (node = el.firstChild)
					{
						lastNode = frag.appendChild(node);
        ...
        observeTT: function()
        {
            this.$editor.find('span.tt').each($.proxy(function(i, s)
            {
                if ($.browser.msie) {
                    $(s).attr('unselectable', 'on');
                }
		...
		toggle: function()
		{
			...
			if (this.opts.visual)
			{
				...
				// Time for a little TT syntax parser to find any rogue
				// html entities within TT tags. We want to convert them back to 
				// standard raw strings.
				var insideTT = false;		// boolean - detect whether we're inside TT tags or not
				var htmllen = html.length;	// integer - the length of the html string so we don't have to continually process it
				for ( var x = 0; x < htmllen; x++ ) {
					var c = html.charAt(x);
					// set insideTT to true if we've hit TT syntaxville
					if (c == '[' && html.charAt(x+1) == '%')
						insideTT = true;

					// set insideTT to false if the closing syntax has been detected
					if (c == '%' && html.charAt(x+1) == ']')
						insideTT = false;

					if (insideTT && c == '&') {
						if (html.charAt(x+1) == 'g' && html.charAt(x+2) == 't' && html.charAt(x+3) == ';') {
							html = html.substr(0, x) + '>' + html.substr(x+4);
						}

						else if (html.charAt(x+1) == 'l' && html.charAt(x+2) == 't' && html.charAt(x+3) == ';') {
							html = html.substr(0, x) + '<' + html.substr(x+4);
						}

						else if (html.charAt(x+1) == ' ' && html.charAt(x-1) == ' ') {
							html = html.substr(0, x) + '&' + html.substr(x+1);
						}

						else if (html.charAt(x+1) == '&') {
							html = html.substr(0, x) + '&&' + html.substr(x+2);
						}
					}
				}
			...
			if ($.editorType == 'gui') {
				var guiToggle = $('#redactor-toggle-gui');
				guiToggle.fadeIn(400, function() { $(this).fadeOut(1000) });
			}
			else if ($.editorType == 'raw') {
				var sourceToggle = $('#redactor-toggle-source');
				sourceToggle.fadeIn(400, function() { $(this).fadeOut(1000) });
			}
		},
        ...
        openTT: function(tt) {
        	var obj = this;
        	var sel = '#redactor_modal #redactor_modal_inner #redactor_modal_content';
            $(tt).hover(function() { $(tt).css('cursor', 'nw-resize'); }, function() { $(tt).css('cursor',''); clicked = false; });
            $(tt).click(function(e) {
            	var me = $(this);
            	var attrs = me.text().match(/(?=\[%).*?\{(.*?)\}.*?(?=%\])/);
            	...
            	if (me.hasClass('tt-element')) {
            		obj.modalInit('Edit Element', '#tt-element-modal', 300, function() { 
            			$(sel).load('/modules/cms/ajax/edit_element/' + me.attr('rel'), { attributes : attrs });
            		});
            	}
            });
        },
		...
		imageSave: function(el)
		{
            ...
            if (parent.hasClass('center-img-wrapper')) {
                $(el).unwrap();
            }
            ...
            {
                $(el).css('float', 'none');
                $(el).wrap('<div class="center-img-wrapper" style="text-align:center"></div>');
            }
			else
			{
	...
	$.fn.insertText = function(html)
	{
		this.data('redactor').insertText(html);
	};
